...
//   try {
//     const { userId } = req.params;
//     const { products } = req.body; // Removed couponId
...
//       // Remove any coupon-related fields if they exist in the schema
//       if (cart.couponDiscount !== undefined) {
//         cart.couponDiscount = 0;
//       }
//       if (cart.appliedCouponId !== undefined) {
//         cart.appliedCouponId = null;
//       }
//       if (cart.chargeCalculations.couponDiscount) {
//         delete cart.chargeCalculations.couponDiscount;
//       }
//     }
...
//     // Now calculate totals for ALL products in cart (after updates)
//     subTotal = 0;
//     totalItems = 0;
//     totalDiscount = 0;
//     totalOriginalPrice = 0;
...
//     // Calculate final amount (no coupon discount)
//     const finalAmount = Number((
//       subTotal + 
//       gstOnFood + 
//       platformChargeAmt + 
...
//     // Prepare response (without coupon data)
//     const responseData = {
//       success: true,
//       message: "Cart updated successfully",
//       cart: cart.toObject(),
...
exports.addToCart = async (req, res) => {
  try {
    ...
    // 🔥 FIX: Use location field instead of addresses 🔥
    if (!user.location || !user.location.coordinates || user.location.coordinates.length === 0) {
      return res.status(400).json({ 
        success: false, 
        message: "User location is required to add items to cart. Please update your profile with location coordinates.",
    ...
    const responseData = {
      ...
      // 🔥 FIX: Updated distance details to use location field 🔥
      distanceDetails: {
        userLocation: {
          coordinates: userLocation.coordinates
        },
